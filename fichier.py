...
def poids_restant_dans_l_ascenceur(ascenceur):
    return ascenceur["poids_max"] - sum(colis["poids"] for colis in ascenceur["colis"])
...
def poids_dans_l_ascenceur(ascenceur):
    return sum(colis["poids"] for colis in ascenceur["colis"])


# Ici on calcule le poids qui est deja dans l'ascenceur


def prendre_colis(ascenceur, colis):
    if colis["poids"] <= poids_restant_dans_l_ascenceur(ascenceur):
        ascenceur["colis"].append(colis)
        return 1
    else:
        return 0
